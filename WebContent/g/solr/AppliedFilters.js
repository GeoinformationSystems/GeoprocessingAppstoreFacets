        ...
        if (qClauses) {
          ...
          for (i=0;i<n;i++) {
            if (qClauses[i].viewable) {
              ...
              if (qClauses[i].urlParameterValue!="id.table_s:table.docindex") {
            	  new AppliedFilter({qClause: qClauses[i]}, domConstruct.create("div",{},oContentNode));
              } else {
            	  new AppliedFilter({qClause: qClauses[i]}, null);
              }
            }
          }
        }
